...
def xor(str1, str2):
    res = ""
    for i in range(0, len(str1)):  # 字符串长度
        xor_res = int(str1[i], 10)^int(str2[i], 10) # 将每个字符转换为十进制之后异或运算
        if xor_res == 1:
            res += '1'
        else:
            res += '0'
...
# 循环左移
def left_run(str, num):  # num 为左移的位数
...
def key_gen(str):
    ...
    key_c = key_change_res[0:28]  # 左半部分
    key_d = key_change_res[28:]   # 右半部分
    num = [0, 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]
    for i in range(1, 17): #共16轮
        key_c = left_run(key_c, num[i])
        key_d = left_run(key_d, num[i])
        key_yiwei = key_c + key_d       # 56位
        key_res = key_change_2(key_yiwei) # 48位
...
def F_function(str, key):         # R(32位)->E盒(48位)->与key的子密钥异或->S盒->P置换
    str_e_res = E_box(str)            # 将 E 异或 S  P 集合到一个函数中，便于调用
    xor_res = xor(str_e_res, key)     # E盒后为48位 与子密钥异或
    str_s_res = S_box(xor_res)        # S盒压缩32位
    str_p_res = P_box(str_s_res)      # P盒置换
    return str_p_res
...
# DES加密操作
def DESencode(text, key, IV):
    text_bin = str_process(text)     # 将字符转换为二进制数
    text_bin = xor(text_bin,IV)
    ...
    for i in range(0, 15):      # 十五轮加密
        ...
        text_right = xor(F_function(text_right, key_list[i]) , text_left) #F 函数的作用 R(32位)->E盒(48位)->与key的子密钥异或(48位)->S盒(32位)->P置换(32位)
        text_left = mes_tmp
...
# 针对一组的解密程序
def DESdecode(text, key, IV):  #密文直接输64位2进制
    ...
    while i > 0:                # 十五轮加密 反过来
        ...
        i = i - 1
    left_text = xor(cipher_left, F_function(cipher_right, key_list[0]))
    right_text = cipher_right
    plain_bin = left_text + right_text
    plain_bin = IP_re(plain_bin)
    plain_bin = xor(plain_bin, IV)
    plain_text = ""
    for i in range(0, len(plain_bin), 8):
        plain_text += chr(int(plain_bin[i:i + 8], 2))
    #plain_text = xor(plain_text,IV)
    return plain_text


def Divide_text(order,text,key):    # 将明文或者明文分组 明文分成8个字符一组，密文则分成64bit一组
    block_text = []
    IV = 'aaaaaaaa'                 # 初始向量，第一轮使用初始向量IV与明文异或后进行加密
    IV_bin = str_process(IV)        # 之后的每一轮使用上一轮的得到的密文与明文异或后加密
    res = ""
    length = 0
    if order == "E":
        length = 8
...
if __name__ == '__main__':
    ...
    # 4e5478ac422415c0
    # 1010011010000111001001001010111111011101110110110101111100101100